...
class BasePage {
  ...
  static getSimulatedDataUrl() {
    if (!BasePage.simulatedDataUrl) {
      const simulatedHtml = `
        <!DOCTYPE html>
        <html>
      ...
      `;
      BasePage.simulatedDataUrl = `data:text/html;charset=utf-8,${encodeURIComponent(simulatedHtml)}`;
    }
    return BasePage.simulatedDataUrl;
  }

  async navigateTo(relativeUrl = '') {
    if (process.env.OFFLINE_MODE || BasePage.forceOffline) {
      await this.driver.get(BasePage.getSimulatedDataUrl());
      return;
    }
