...
export default function QuestionnaireScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to save clinical baseline.');
      }

      // Update state store with complete profile data
      useAuthStore.setState({ user: data.userProfile });

      haptics.success();
      router.replace('/(tabs)');
    } catch (err: any) {
      console.warn('[Questionnaire] Error during submit:', err);
        ...
        <View style={styles.card}>
          ...
          <View style={styles.header}>
            ...
            <Text style={[styles.subtitle, { color: colors.textSub }]}>
              Step {currentStep} of 5 · Help us customize your clinical experience & reports.
            </Text>
          </View>
              ...
              {isLoading ? (
                ...
                <>
                  <Text style={styles.submitText}>
                    {currentStep === 5 ? 'Complete Baseline' : 'Continue'}
                  </Text>
                  <Feather name={currentStep === 5 ? 'check' : 'arrow-right'} size={18} color="#ffffff" />
                </>
              )}
            </TouchableOpacity>
          </View>
