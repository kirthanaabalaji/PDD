...
export default function ReportsScreen() {
  ...
  // ── Weekly Report Limit (Disabled for unlimited report generation) ─────────
  ...
  const isLimitReached = false; // Unlimited report generation allowed
  ...
  const handlePdfDownload = async () => {
    ...
    try {
      ...
      const summaryText = aiReport?.executiveSummary || aiReport?.clinicalSummary || `The patient's recent health tracking records indicate a concerning trend of wheezing and respiratory distress, particularly in the presence of known triggers such as ${patientTriggers}. ${patientSeverity} asthma symptoms have been reported, but the severity has increased over time. It is essential to reassess the patient's treatment plan and consider the prescription of rescue medication to manage acute symptoms.`;
      
      const triggerText = aiReport?.triggerAnalysis || `The patient's exposure to ${patientTriggers} appears to be a significant trigger for wheezing and respiratory distress. It is crucial to identify and avoid these triggers to prevent exacerbations. Environmental modifications, such as using air purifiers or avoiding areas with high trigger levels, may be beneficial.`;
      
      const complianceText = aiReport?.exerciseEvaluation || aiReport?.complianceEvaluation || `The patient has demonstrated inconsistent adherence to breathing exercises, which may be contributing to the worsening of symptoms. Encouraging regular practice of pursed-lip breathing and box breathing exercises may help improve lung function and reduce symptoms.`;
      
      const actionItems = (aiReport?.actionItems || aiReport?.actionPlan) && (aiReport?.actionItems || aiReport?.actionPlan).length > 0
        ? (aiReport?.actionItems || aiReport?.actionPlan)
        : [
            'Prescribe rescue medication (e.g., albuterol inhaler) for acute symptom management.',
            `Recommend environmental modifications to minimize exposure to ${patientTriggers}.`,
      ...
      const actionItemsHtml = actionItems.map((item: string) => `
      ...
      const htmlContent = `
        ...
        <body>
          <div class="header">
            ...
            <div class="report-date">Date: ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
          </div>
          ...
          <table class="patient-table">
            <tr>
              ...
              <td class="val">${patientAge} / -</td>
            </tr>
            <tr>
              <td class="label">SEVERITY STAGE</td>
              <td class="val">${patientSeverity}</td>
              <td class="label">PRESCRIBED INHALER</td>
              <td class="val">${patientInhaler}</td>
            </tr>
            <tr>
              <td class="label">KNOWN TRIGGERS</td>
              <td class="val" colspan="3">${patientTriggers}</td>
            </tr>
          </table>
            ...
            <View style={{ gap: 10 }}>
              ...
              <View style={[styles.reportSection, { backgroundColor: colors.accentDim, paddingVertical: 10, paddingHorizontal: 12, borderRadius: 10 }]}>
                ...
                <Text style={[styles.sectionBodyText, { color: colors.text, fontSize: 13, marginTop: 6, lineHeight: 18 }]} numberOfLines={2}>
                  {aiReport?.executiveSummary || aiReport?.clinicalSummary || "The patient's recent health tracking records indicate a concerning trend of wheezing and respiratory distress, particularly in the presence of known triggers such as Smoke and Dust."}
                </Text>
              </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16, paddingBottom: 10 }}>
              ...
              <View style={{ backgroundColor: colors.bg, padding: 12, borderRadius: 12, borderWidth: 1, borderColor: colors.cardBorder, gap: 4 }}>
                <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 13, color: colors.text }}>
                  Patient: {user?.name || 'Kirthanaa'}
                </Text>
                <Text style={{ fontFamily: 'Inter_500Medium', fontSize: 11, color: colors.textSub }}>
                  Age: {user?.profile?.age ? String(user.profile.age) : '21'} · Severity: {user?.profile?.severity || 'Mild'}
                </Text>
              </View>
              ...
              <View style={[styles.reportSection, { backgroundColor: colors.accentDim, padding: 12, borderRadius: 10 }]}>
                ...
                <Text style={[styles.sectionBodyText, { color: colors.text, fontSize: 13, lineHeight: 18, marginTop: 8 }]}>
                  {aiReport?.executiveSummary || aiReport?.clinicalSummary || `The patient's recent health tracking records indicate a concerning trend of wheezing and respiratory distress, particularly in the presence of known triggers such as Smoke and Dust. Mild asthma symptoms have been reported, but the severity has increased over time. It is essential to reassess the patient's treatment plan and consider the prescription of rescue medication to manage acute symptoms.`}
                </Text>
              </View>
              ...
              <View style={[styles.reportSection, { backgroundColor: colors.bg, padding: 12, borderRadius: 10, borderWidth: 1, borderColor: colors.cardBorder }]}>
                ...
                <Text style={[styles.sectionBodyText, { color: colors.text, fontSize: 13, lineHeight: 18, marginTop: 8 }]}>
                  {aiReport?.triggerAnalysis || `The patient's exposure to Smoke and Dust appears to be a significant trigger for wheezing and respiratory distress. It is crucial to identify and avoid these triggers to prevent exacerbations. Environmental modifications, such as using air purifiers or avoiding areas with high smoke and dust levels, may be beneficial.`}
                </Text>
              </View>
              ...
              <View style={[styles.reportSection, { backgroundColor: colors.bg, padding: 12, borderRadius: 10, borderWidth: 1, borderColor: colors.cardBorder }]}>
                ...
                <View style={{ marginTop: 10, gap: 10 }}>
                  {((aiReport?.actionItems || aiReport?.actionPlan) && (aiReport?.actionItems || aiReport?.actionPlan).length > 0
                    ...
                    : [
                        'Prescribe rescue medication (e.g., albuterol inhaler) for acute symptom management.',
                        'Recommend environmental modifications to minimize exposure to smoke and dust.',
                        'Encourage regular practice of pursed-lip breathing and box breathing exercises to improve lung function and reduce symptoms.',
                      ]
