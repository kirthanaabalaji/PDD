...
import { useAuthStore, useSessionStore, useSymptomStore, useAnalysisStore, isRealToken } from '../../src/store';
...
export default function ProfileScreen() {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (isRealToken(token)) {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            ...
            setAlertConfig({
              ...
              message: 'Your authentication token is invalid or expired. Logging you out.',
              type: 'error',
              onClose: () => {
                logout();
                navigation.getParent()?.reset({
      ...
      // Standalone demo mode / local session fallback
      haptics.success();
      const isLocal = token?.startsWith('local_auth_token_');
      setAlertConfig({
        visible: true,
        title: 'Account Deleted',
        message: isLocal
          ? 'Local session data successfully purged from the device.'
          : 'Demo session data successfully purged from the device.',
        type: 'success',
        onClose: () => {
          logout();
          navigation.getParent()?.reset({
