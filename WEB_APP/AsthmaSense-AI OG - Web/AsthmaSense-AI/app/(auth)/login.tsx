...
export default function LoginScreen() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('Backend login response not ok, attempting local session authentication');
      ...
      console.warn('[Login] Error during submit, falling back to local session:', err);
      const fallbackProfile = {
        id: 'user_' + Date.now(),
        name: trimmedEmail.split('@')[0] || 'User',
        email: trimmedEmail,
